...
The site renders these annotations as furigana, but it must not infer readings automatically because kanji readings depend on context. After class-content changes, run `deno task check` and `deno task build`, then verify the rendered page at desktop and mobile widths.
